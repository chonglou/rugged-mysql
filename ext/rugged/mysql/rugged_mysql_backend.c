...
static void
rb_rugged_mysql_backend__free (rugged_mysql_backend * backend)
{
  free (backend->host);
  free (backend->socket);
...
static rugged_mysql_backend *
rugged_mysql_backend_new (char *host, int port,
			  char *socket,
...
*/
static VALUE
rb_rugged_mysql_backend_new (VALUE klass, VALUE rb_opts)
{
  VALUE val;
  char *host;
...
void
Init_rugged_mysql_backend (void)
{
  rb_cRuggedMysqlBackend =
    rb_define_class_under (rb_mRuggedMysql, "Backend", rb_cRuggedBackend);
