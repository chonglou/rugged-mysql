...
static int rugged_mysql_odb_backend(git_odb_backend** backend_out, rugged_backend* backend, const char* path){
    //todo
    return -1;
}

static int rugged_mysql_refdb_backend(git_refdb_backend** backend_out, rugged_backend* backend, const char* path){
    //todo
    return -1;
}

static rugged_mysql_backend *rugged_mysql_backend_new(char* host, int port, char* socket, char* username, char* password, char* database) {
    rugged_mysql_backend* mysql_backend = malloc(sizeof(rugged_mysql_backend));
...
static VALUE rb_rugged_mysql_backend_new(VALUE klass, VALUE rb_opts)
{
    ...
    return Data_Wrap_Struct(klass, NULL, rb_rugged_mysql_backend__free, rugged_mysql_backend_new(host, port, socket, username, password, database));
}
