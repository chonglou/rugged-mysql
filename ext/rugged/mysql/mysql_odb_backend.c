  ...
  if (mysql_stmt_num_rows (backend->st_read_header) == 1)
    {
      ...
      if (mysql_stmt_bind_result (backend->st_read_header, result_buffers) !=
	  0)
	{
	  return GIT_ERROR;
	}
...
int
mysql_odb_backend__exists (git_odb_backend * _backend, const git_oid * oid)
{
  mysql_odb_backend *backend;
  int found;
  MYSQL_BIND bind_buffers[1];
...
void
mysql_odb_backend__free (git_odb_backend * _backend)
{
  mysql_odb_backend *backend;
  assert (_backend);
  backend = (mysql_odb_backend *) _backend;
...
static int
init_statements (mysql_odb_backend * backend)
{
  my_bool truth = 1;

  static const char *sql_read =
    "SELECT `type`, `size`, UNCOMPRESS(`data`) FROM `" GIT2_ODB_TABLE_NAME
    "` WHERE `oid` = ?;";
